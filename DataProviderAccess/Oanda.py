...
OandaSymbols = ['AU200_AUD', 'AUD_CAD', 'AUD_CHF', 'AUD_HKD', 'AUD_JPY', 'AUD_NZD', 'AUD_SGD', 'AUD_USD', 'BCO_USD',
                ...
                'NAS100_USD', 'NATGAS_USD', 'NL25_EUR', 'NZD_CAD', 'NZD_CHF', 'NZD_HKD', 'NZD_JPY', 'NZD_SGD',
                'NZD_USD',
...
def getDataFromStartDate(start_date, instrument='WTICO_USD', gran='M15', day_diff=5):
    ...
    print(instrument)
    print(start_date)
    start_date_string = str(start_date.year) + '-' + '{:02d}'.format(start_date.month) + '-' + '{:02d}'.format(
        start_date.day)
    ...
    while condition:
        try:
            df = pd.DataFrame(
                OANDA.get_history(instrument=instrument, granularity=gran, start=start_date_string, count=5000)[
                    'candles'])
...
def resample_dohlcv_pandas(DF, targetTF):
    ...
    ResampledData = DF[['Open', 'High', 'Low', 'Close']].resample(targetTF, closed='left', label='left',
                                                                  how=ohlc_dict)  # .apply(ohlc_dict)
...
class MyTestCase(unittest.TestCase):
    ...
    def test_Oanda(self):
        ...
        print(StartDateString)
        print(pd.DataFrame(
            OANDA.get_history(instrument='WTICO_USD', start=StartDateString, granularity='H1', count=5000)['candles']))
